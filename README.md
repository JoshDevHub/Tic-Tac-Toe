...
For `stdout`, I just make sure that the `CLI` `stdout` is writing to a [`StringIO` object](https://github.com/JoshDevHub/Tic-Tac-Toe/blob/add-e2e-tests/spec/tic_tac_toe/cli_spec.rb#L13). Every time `CLI` calls `puts` or `print`, it will write to this object. Checking that certain substrings have appeared in the output is very convenient with `StringIO`. You just call `StringIO#string` and use the `include` matcher. I consider this far easier to work with and read compared to RSpec's terminal output matcher.

#### A Note About E2E Testing

So I only wrote two tests. Ideally there'd be another to check for a draw, and maybe one to check that players can play multiple games. But it's important with E2E testing that you don't go hunting down every little edge case. Your unit testing of your domain objects should handle that. E2E/system testing is for making sure the core flow the user goes through is running smoothly.
